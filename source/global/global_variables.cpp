...
QString Global_Variables::parameter_Name(const Data &struct_Data, QString whats_This, int index)
{
		...
		struct_Data.item_Type == item_Type_Substrate  )
	{
		...
		if(whats_This == whats_This_PSD_Exponenta_Mu)						text = struct_Data.material + " " + brackets + " Correlation depth, " + "L"+Subscript_v_Sym;
	...
	if(	struct_Data.item_Type == item_Type_Layer )
	{
		...
		if(whats_This == whats_This_Particle_Cross_Layer_Deviation)		text = struct_Data.material + " " + brackets + " Particle cross-layer deviation, " + Delta_Big_Sym;
...
Parameter* Global_Variables::get_Parameter_From_Struct_Item_by_Whats_This(Data& struct_Data, QString whats_This, int* line_edit_precision, int* thumbnail_precision, QString* units, double* coeff)
{
	...
	// particles
	if(whats_This == whats_This_Particle_Composition)					{*line_edit_precision = line_edit_composition_precision;					*thumbnail_precision = thumbnail_composition_precision;					*units = "";							*coeff = 1;																		return nullptr;															}
	...
	if(whats_This == whats_This_Particle_Radius)						{*line_edit_precision = line_edit_particle_size_precision;					*thumbnail_precision = thumbnail_particle_size_precision;				*units = " " + length_units;			*coeff = length_Coefficients_Map.value(length_units);							return &struct_Data.particles_Model.particle_Radius;					}
	if(whats_This == whats_This_Particle_Height)						{*line_edit_precision = line_edit_particle_size_precision;					*thumbnail_precision = thumbnail_particle_size_precision;				*units = " " + length_units;			*coeff = length_Coefficients_Map.value(length_units);							return &struct_Data.particles_Model.particle_Height;					}
...
double Global_Variables::PSD_Real_Gauss_1D_Finite_from_nu(double sigma, double xi, double alpha, double p, double a, gsl_spline *spline, gsl_interp_accel *acc)
{
	Q_UNUSED(alpha)
	Q_UNUSED(spline)
	Q_UNUSED(acc)
	double val = M_PI*p*xi;
	return 2*sqrt(M_PI) * sigma*sigma*xi * exp(-val*val) * erf(sqrt(abs(a*a-p*p))*M_PI*xi);
}
...
double Global_Variables::PSD_Fractal_Gauss_1D(double factor, double xi, double alpha, double k, double cos_Theta, double cos_Theta_0, gsl_spline* spline, gsl_interp_accel* acc)
{
	Q_UNUSED(factor)
	Q_UNUSED(xi)
	Q_UNUSED(alpha)
...
void Global_Variables::plot_All_Data_in_Particles()
{
	if(global_Multilayer_Approach->runned_Particles_Plots_Window.contains(particles_Plots_Key))
	{
		for(Particles_Plot* particles_Plot : global_Multilayer_Approach->particles_Plots_Window->particles_Plot_Vector)
		{
			particles_Plot->plot_Data(true);
		}
	}
}

void Global_Variables::variable_Drift(double& value, const Drift &drift, int period_Index, int num_Repetition, gsl_rng* r)
{
...
// taken from BornAgain:
// https://jugit.fz-juelich.de/mlz/bornagain
QCPColorGradient color_Gradient_Inferno()
{
