#include "particles_plots_window.h"
...
void Particles_Plots_Window::create_Main_Layout()
{
...
void Particles_Plots_Window::set_Window_Geometry()
{
	int width_add = 0, height_add = 0;
	if(make_all_windows_resizeable && !previous_all_windows_resizeable) {width_add+=2; height_add+=2;}

	setGeometry(particles_plots_x_corner,particles_plots_y_corner,particles_plots_width+width_add,particles_plots_height+height_add);
}

void Particles_Plots_Window::write_Window_Geometry()
{
	// usual show
	if(!isMaximized())
	{
		particles_plots_x_corner = frameGeometry().x()-corner_x_shift;
		particles_plots_y_corner = frameGeometry().y()-corner_y_shift;

		particles_plots_width  = geometry().width() +  width_add;
		particles_plots_height = geometry().height()+ height_add;
	}

}

void Particles_Plots_Window::create_Tabs()
{
...
void Particles_Plots_Window::add_Tabs()
{
	particles_Plot_Vector.resize(global_Multilayer_Approach->multilayer_Tabs->count());
...
void Particles_Plots_Window::create_Tab_Content(QWidget* new_Widget, int tab_Index)
{
	Multilayer* multilayer = qobject_cast<Multilayer*>(global_Multilayer_Approach->multilayer_Tabs->widget(tab_Index));

	Particles_Plot* new_Plot = new Particles_Plot(multilayer, this, this);
		particles_Plot_Vector[tab_Index] = new_Plot;
