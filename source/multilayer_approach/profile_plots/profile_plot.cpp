...
{
	double many_Points_Profile_Line_Thickness = default_Profile_Line_Thickness;
...
//	gsl_integration_workspace_free(w);
	...
	{		
		...
		if(multilayer->profile_Plot_Options.show_Sharp_Profile)
		{
			...
			else									custom_Plot->graph()->setPen(QPen(Qt::darkCyan, many_Points_Profile_Line_Thickness));
...
//				val.prepend(delta_To_Plot_Vector.first().value);
			...
			else									custom_Plot->graph()->setPen(QPen(Qt::blue, many_Points_Profile_Line_Thickness));
	...
	if(multilayer->profile_Plot_Options.type == PERMITTIVITY && multilayer->profile_Plot_Options.permittivity_Type == BETA_EPS	)
	{
		...
		if(multilayer->profile_Plot_Options.show_Sharp_Profile)
		{
			...
			else									custom_Plot->graph()->setPen(QPen(Qt::darkMagenta, many_Points_Profile_Line_Thickness));
...
//				val.prepend(beta_To_Plot_Vector.first().value);
			...
			else									custom_Plot->graph()->setPen(QPen(Qt::red, many_Points_Profile_Line_Thickness));
	...
	if(multilayer->profile_Plot_Options.type == MATERIAL)
	{
		...
		for(int material_index = 0; material_index<different_Materials.size(); material_index++)
		{
			...
			if(multilayer->profile_Plot_Options.show_Sharp_Profile)
			{
				...
				else									custom_Plot->graph()->setPen(QPen(color_Contrast_Adjoint_Sequence[material_index%color_Contrast_Adjoint_Sequence.size()], many_Points_Profile_Line_Thickness));
...
//					val_Multiple[material_index].prepend(materials_To_Plot_Vector_Vector[material_index].first().value);
				...
				else								    custom_Plot->graph()->setPen(QPen(color_Contrast_Sequence[material_index%color_Contrast_Sequence.size()], many_Points_Profile_Line_Thickness));
	...
	if(multilayer->profile_Plot_Options.type == ELEMENTS)
	{
		...
		for(int element_Index = 0; element_Index<different_Elements.size(); element_Index++)
		{
			...
			if(multilayer->profile_Plot_Options.show_Sharp_Profile)
			{
				...
				else									custom_Plot->graph()->setPen(QPen(color_Contrast_Adjoint_Sequence[element_Index%color_Contrast_Adjoint_Sequence.size()], many_Points_Profile_Line_Thickness));
...
//					val_Multiple[element_Index].prepend(elements_To_Plot_Vector_Vector[element_Index].first().value);
				...
				else									custom_Plot->graph()->setPen(QPen(color_Contrast_Sequence[element_Index%color_Contrast_Sequence.size()], many_Points_Profile_Line_Thickness));
...
void Profile_Plot::print_To_File(QString path)
{
    QFile file(path);
...
void Profile_Plot::print_Profile(QTextStream& out)
{
    ...
    /// headline
    {
        // top header
        {
            ...
            if(multilayer->profile_Plot_Options.type == PERMITTIVITY)
            {
                out << endl << "; " << at_Fixed_Heading << endl;
            }

            if(multilayer->profile_Plot_Options.show_Sharp_Profile)
            {
                out << endl << "; Sharp profile: arguments correspond to the top layer boundaries";
                out << endl << "; For visualization please use step line with data point on the left of the step" << endl;
            } else
            {
                if(multilayer->discretization_Parameters.enable_Discretization && multilayer->profile_Plot_Options.show_Discretization)
                {
                    out << endl << "; Discretized profile: arguments correspond to the top slice boundaries";
                    out << endl << "; For visualization please use step line with data point on the left of the step" << endl;
                } else
                {
