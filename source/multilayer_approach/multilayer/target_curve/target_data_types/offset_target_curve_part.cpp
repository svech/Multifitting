#include "offset_target_curve_part.h"

Offset_Target_Curve_Part::Offset_Target_Curve_Part(Target_Curve* target_Curve, Target_Curve_Editor* target_Curve_Editor, QWidget *parent) :
...
void Offset_Target_Curve_Part::create_Argument_GroupBox()
{
	// initialization
...
void Offset_Target_Curve_Part::create_Value_GroupBox()
{
	// initialization
...
void Offset_Target_Curve_Part::create_Beam_GroupBox()
{
	...
	// at fixed offset
	{
		...
		beam_GroupBox_Layout->addWidget(at_Fixed_Detector_Offset_Label,1,0,1,2,Qt::AlignLeft);
		...
		at_Fixed_Detector_Offset_Units_Label = new QLabel(target_Curve->angular_Units);
		beam_GroupBox_Layout->addWidget(at_Fixed_Detector_Offset_Units_Label,1,4,Qt::AlignLeft);
	}
	// background
	{
...
void Offset_Target_Curve_Part::reset_Subinterval()
{
	if(target_Curve->loaded_And_Ready)
	{
		double min = *std::min_element(target_Curve->curve.shifted_Argument.begin(), target_Curve->curve.shifted_Argument.end());
...
void Offset_Target_Curve_Part::fill_At_Fixed_Wavelength_Label()
{
...
void Offset_Target_Curve_Part::fill_Spectral_Width_Label()
{
	if(	target_Curve->spectral_Units == wavelength_Units_List[angstrom] ||
		target_Curve->spectral_Units == wavelength_Units_List[nm]	   )
	{
...
void Offset_Target_Curve_Part::refresh_At_Fixed_Wavelength_Units()
{
...
void Offset_Target_Curve_Part::refresh_At_Fixed_Detector_Offset_Value()
{
	double coeff = angle_Coefficients_Map.value(target_Curve->angular_Units);
	target_Curve->measurement.detector_Theta_Offset = at_Fixed_Detector_Offset_SpinBox->value()*coeff;
...
void Offset_Target_Curve_Part::refresh_At_Fixed_Wavelength_Value()
{
	double coeff = wavelength_Coefficients_Map.value(target_Curve->spectral_Units);
...
void Offset_Target_Curve_Part::refresh_Plot_Axes_Labels()
{
...
void Offset_Target_Curve_Part::connecting()
{
	/// argument box
	// argument units
	...
	{
		refresh_At_Fixed_Detector_Offset_Value();
		global_Multilayer_Approach->global_Recalculate();
	});
	// at fixed wavelength
