#include "rocking_target_curve_part.h"

Rocking_Target_Curve_Part::Rocking_Target_Curve_Part(Target_Curve* target_Curve, Target_Curve_Editor* target_Curve_Editor, QWidget *parent) :
...
void Rocking_Target_Curve_Part::create_Argument_GroupBox()
{
	// initialization
	...
	// argument type
	{
		arg_Type_ComboBox = new QComboBox;
			...
			arg_Type_ComboBox->addItem(argument_Types[Deviation_From_Specular_Angle]);
...
void Rocking_Target_Curve_Part::create_Value_GroupBox()
{
	// initialization
...
void Rocking_Target_Curve_Part::create_Beam_GroupBox()
{
	...
	// at fixed specular position
	{
		...
		beam_GroupBox_Layout->addWidget(at_Fixed_Specular_Position_Label,1,0,1,2,Qt::AlignLeft);

		// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

		double coeff = angle_Coefficients_Map.value(target_Curve->angular_Units);

		at_Fixed_Specular_Position_SpinBox = new MyDoubleSpinBox;
			at_Fixed_Specular_Position_SpinBox->setAccelerated(true);
			at_Fixed_Specular_Position_SpinBox->setRange(0,90./coeff);
			at_Fixed_Specular_Position_SpinBox->setDecimals(7);
			at_Fixed_Specular_Position_SpinBox->setSingleStep(0.01);
			at_Fixed_Specular_Position_SpinBox->setValue(target_Curve->measurement.beam_Theta_0_Specular_Position/coeff);
			at_Fixed_Specular_Position_SpinBox->setButtonSymbols(QAbstractSpinBox::NoButtons);
			at_Fixed_Specular_Position_SpinBox->setProperty(min_Size_Property, TARGET_LINE_AT_FIXED_WIDTH);
		beam_GroupBox_Layout->addWidget(at_Fixed_Specular_Position_SpinBox,1,2,1,2,Qt::AlignRight);
		Global_Variables::resize_Line_Edit(at_Fixed_Specular_Position_SpinBox);

		// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

		at_Fixed_Specular_Position_Units_Label = new QLabel(target_Curve->angular_Units);
		beam_GroupBox_Layout->addWidget(at_Fixed_Specular_Position_Units_Label,1,4,Qt::AlignLeft);
	}
	// background
	{
...
void Rocking_Target_Curve_Part::reset_Subinterval()
{
	if(target_Curve->loaded_And_Ready)
	{
		double min = *std::min_element(target_Curve->curve.shifted_Argument.begin(), target_Curve->curve.shifted_Argument.end());
...
void Rocking_Target_Curve_Part::refresh_Argument_Units()
{
	...
	// specular position
	reset_Range_Specular_Position();
	at_Fixed_Specular_Position_Units_Label->setText(target_Curve->angular_Units);
...
void Rocking_Target_Curve_Part::fill_At_Fixed_Wavelength_Label()
{
...
void Rocking_Target_Curve_Part::fill_Spectral_Width_Label()
{
	if(	target_Curve->spectral_Units == wavelength_Units_List[angstrom] ||
		target_Curve->spectral_Units == wavelength_Units_List[nm]	   )
	{
...
void Rocking_Target_Curve_Part::refresh_At_Fixed_Wavelength_Units()
{
...
void Rocking_Target_Curve_Part::refresh_At_Fixed_Specular_Position()
{
	double coeff = angle_Coefficients_Map.value(target_Curve->angular_Units);
	target_Curve->measurement.beam_Theta_0_Specular_Position = at_Fixed_Specular_Position_SpinBox->value()*coeff;
...
void Rocking_Target_Curve_Part::refresh_At_Fixed_Wavelength_Value()
{
	double coeff = wavelength_Coefficients_Map.value(target_Curve->spectral_Units);
...
void Rocking_Target_Curve_Part::refresh_Plot_Axes_Labels()
{
	...
	target_Curve_Plot->argument_Type_Text = target_Curve->measurement.argument_Type;
...
void Rocking_Target_Curve_Part::connecting()
{
	/// argument box
	// argument type
	...
	/// beam box
	// at fixed
	connect(at_Fixed_Specular_Position_SpinBox, static_cast<void (MyDoubleSpinBox::*)(double)>(&MyDoubleSpinBox::valueChanged), this, [=]
	{
		refresh_At_Fixed_Specular_Position();
		global_Multilayer_Approach->global_Recalculate();
	});
	// at fixed wavelength
