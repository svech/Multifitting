	...
	if(check)
	{
		if(type_Curve->measurement.measurement_Type == no_Measurement_Type)
		{
            ...
            QPushButton* gisas_Button    = new QPushButton(measurement_Types[GISAS_Map]);
                connect(gisas_Button,     &QPushButton::clicked, this, [=]{
                    type_Curve->measurement.measurement_Type = measurement_Types[GISAS_Map];
                    type_Curve->calc_Functions.instrumental_Smoothing = false;
                    choice_Data_Type_Window->close();
                    open_Editor_Window(type_Curve, type_Curve_Editor);});
		...
		} else
		{
			...
			if(typeid(Type_Curve) == typeid(Independent_Curve))
			{
				...
				if(independent_Curve->measurement.measurement_Type == measurement_Types[GISAS_Map])
				{
					...
					{
						QString text;
//						if( imperfections_Model.PSD_Model == stretched_exp_Model ) text = "2D scattering can't be simulated with stretched exp model of roughness.\nChange the model in Structure Table.";
