#include "rocking_independent_curve_part.h"

Rocking_Independent_Curve_Part::Rocking_Independent_Curve_Part(Independent_Curve* independent_Curve, QWidget *parent) :
...
void Rocking_Independent_Curve_Part::create_Argument_GroupBox()
{
	// initialization
...
void Rocking_Independent_Curve_Part::create_Beam_GroupBox()
{
	...
	// at fixed beam theta_0
	{
		...
		beam_GroupBox_Layout->addWidget(at_Fixed_Specular_Position_Label,0,0,Qt::AlignLeft);
		...
		beam_GroupBox_Layout->addWidget(at_Fixed_Specular_Position_SpinBox,0,1,Qt::AlignLeft);
		Global_Variables::resize_Line_Edit(at_Fixed_Specular_Position_SpinBox);

		// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

		at_Fixed_Specular_Position_Units_Label = new QLabel(independent_Curve->angular_Units);
		beam_GroupBox_Layout->addWidget(at_Fixed_Specular_Position_Units_Label,0,2,1,3,Qt::AlignLeft);
	}
	// at fixed wavelength
	{
...
void Rocking_Independent_Curve_Part::fill_Argument()
{
...
void Rocking_Independent_Curve_Part::refresh_Angular_Units()
{
	start_Argument_Spinbox->blockSignals(true);
	final_Argument_Spinbox->blockSignals(true);
...
void Rocking_Independent_Curve_Part::refresh_Spectral_Units()
{
...
void Rocking_Independent_Curve_Part::refresh_At_Fixed_Wavelength_Value()
{
	double coeff = wavelength_Coefficients_Map.value(independent_Curve->spectral_Units);
...
void Rocking_Independent_Curve_Part::refresh_At_Fixed_Specular_Position()
{
	double coeff = angle_Coefficients_Map.value(independent_Curve->angular_Units);

	double old_Position = independent_Curve->measurement.beam_Theta_0_Specular_Position;
	independent_Curve->measurement.beam_Theta_0_Specular_Position = at_Fixed_Specular_Position_SpinBox->value()*coeff;
	double delta = independent_Curve->measurement.beam_Theta_0_Specular_Position - old_Position;
...
void Rocking_Independent_Curve_Part::fill_Spectral_Width_Label()
{
	if(	independent_Curve->spectral_Units == wavelength_Units_List[angstrom] ||
			independent_Curve->spectral_Units == wavelength_Units_List[nm]	   )
	{
...
void Rocking_Independent_Curve_Part::connecting()
{
