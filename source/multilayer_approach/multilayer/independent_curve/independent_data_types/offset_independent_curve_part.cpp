#include "offset_independent_curve_part.h"

Offset_Independent_Curve_Part::Offset_Independent_Curve_Part(Independent_Curve* independent_Curve, QWidget *parent) :
...
void Offset_Independent_Curve_Part::create_Units_GroupBox()
{
...
void Offset_Independent_Curve_Part::create_Beam_GroupBox()
{
...
void Offset_Independent_Curve_Part::fill_Offset()
{
	at_Fixed_Detector_Offset_SpinBox->blockSignals(true);

	double coeff = angle_Coefficients_Map.value(independent_Curve->angular_Units);

	double min_Range =     -min(independent_Curve->measurement.beam_Theta_0_Angle.independent.min,independent_Curve->measurement.beam_Theta_0_Angle.independent.max);
	double max_Range = 180.-max(independent_Curve->measurement.beam_Theta_0_Angle.independent.min,independent_Curve->measurement.beam_Theta_0_Angle.independent.max);
	at_Fixed_Detector_Offset_SpinBox->setRange(min_Range/coeff,max_Range/coeff);

	independent_Curve->measurement.detector_Theta_Offset = min(independent_Curve->measurement.detector_Theta_Offset, max_Range);
	independent_Curve->measurement.detector_Theta_Offset = max(independent_Curve->measurement.detector_Theta_Offset, min_Range);

	at_Fixed_Detector_Offset_SpinBox->setValue(independent_Curve->measurement.detector_Theta_Offset/coeff);
	at_Fixed_Detector_Offset_Units_Label->setText(independent_Curve->angular_Units);
...
void Offset_Independent_Curve_Part::refresh_Argument_Values(bool num_Points_Changed)
{
	double coeff = angle_Coefficients_Map.value(independent_Curve->angular_Units);
...
void Offset_Independent_Curve_Part::refresh_At_Fixed_Wavelength_Value()
{
	double coeff = wavelength_Coefficients_Map.value(independent_Curve->spectral_Units);
...
void Offset_Independent_Curve_Part::refresh_At_Fixed_Offset_Value()
{
	double coeff = angle_Coefficients_Map.value(independent_Curve->angular_Units);
	independent_Curve->measurement.detector_Theta_Offset = at_Fixed_Detector_Offset_SpinBox->value()*coeff;
...
void Offset_Independent_Curve_Part::fill_At_Fixed_Wavelength_Label()
{
...
void Offset_Independent_Curve_Part::fill_Spectral_Width_Label()
{
	if(	independent_Curve->spectral_Units == wavelength_Units_List[angstrom] ||
			independent_Curve->spectral_Units == wavelength_Units_List[nm]	   )
	{
...
void Offset_Independent_Curve_Part::connecting()
{
	...
	/// beam box
	// at fixed beam
	connect(at_Fixed_Detector_Offset_SpinBox, static_cast<void (MyDoubleSpinBox::*)(double)>(&MyDoubleSpinBox::valueChanged), this, [=]
	{
		refresh_At_Fixed_Offset_Value();
		global_Multilayer_Approach->global_Recalculate();
	});
	// at fixed wavelength
