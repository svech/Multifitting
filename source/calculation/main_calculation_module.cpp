...
void Main_Calculation_Module::wrap_With_Specular_Single(Calculated_Values& calculated_Values, const Data& measurement)
{
	...
	double beam_Angular_Divergence_Range_Limit = 3;
	...
	if(beam_Angular_Divergence_Function == distributions[Gaussian])	{beam_Angular_Distribution = Global_Variables::distribution_Gaussian; beam_Angular_Divergence_Norm_Factor = sqrt(log(16.)/M_PI)/beam_Angular_Divergence_FWHM;	beam_Angular_Divergence_Range_Limit = 3;    } else return;
	...
	auto f_D = [&](double point_Index)
	{
		...
		{
			integral = sqrt_pi_l / sqrt(pow(detector_Angular_Width_FWHM,-2) + pow(beam_Angular_Divergence_FWHM,-2)) * pow(2.,-1-4*distance_t_t0*distance_t_t0/(detector_Angular_Width_FWHM*detector_Angular_Width_FWHM + beam_Angular_Divergence_FWHM*beam_Angular_Divergence_FWHM));
		} else
		if(beam_Angular_Divergence_Function == distributions[Gaussian] && detector_Sensitivity_Function == distributions[Gate])
		{
			integral = 0.25 * beam_Angular_Divergence_FWHM * sqrt_pi_l * (erf(log_2s*(detector_Angular_Width_FWHM-2*distance_t_t0)/beam_Angular_Divergence_FWHM)+erf(log_2s*(detector_Angular_Width_FWHM+2*distance_t_t0)/beam_Angular_Divergence_FWHM));
		} else
		if(beam_Angular_Divergence_Function == distributions[Gate] && detector_Sensitivity_Function == distributions[Gaussian])
		{
			integral = 0.25 * detector_Angular_Width_FWHM * sqrt_pi_l * (erf(log_2s*(beam_Angular_Divergence_FWHM-2*distance_t_t0)/detector_Angular_Width_FWHM)+erf(log_2s*(beam_Angular_Divergence_FWHM+2*distance_t_t0)/detector_Angular_Width_FWHM));
		} else
		if(beam_Angular_Divergence_Function == distributions[Gate] && detector_Sensitivity_Function == distributions[Gate])
		{
			if(((detector_Angular_Width_FWHM == beam_Angular_Divergence_FWHM) && ((detector_Angular_Width_FWHM + 2*theta  ) == (beam_Angular_Divergence_FWHM + 2*theta_0))) ||
			   ((beam_Angular_Divergence_FWHM <  detector_Angular_Width_FWHM) && ((detector_Angular_Width_FWHM + 2*theta_0)  > (beam_Angular_Divergence_FWHM + 2*theta  )) &&
				((detector_Angular_Width_FWHM + 2*theta) >= (beam_Angular_Divergence_FWHM + 2*theta_0))))
			{
				integral = beam_Angular_Divergence_FWHM;
			} else
			if(	(beam_Angular_Divergence_FWHM > detector_Angular_Width_FWHM) &&
				((detector_Angular_Width_FWHM + 2*theta  ) < (beam_Angular_Divergence_FWHM + 2*theta_0)) &&
				((detector_Angular_Width_FWHM + 2*theta_0) < (beam_Angular_Divergence_FWHM + 2*theta  )))
			{
				integral = detector_Angular_Width_FWHM;
			} else
			if( ((detector_Angular_Width_FWHM + beam_Angular_Divergence_FWHM + 2*theta) > 2*theta_0) &&
				((((detector_Angular_Width_FWHM + 2*theta) < (beam_Angular_Divergence_FWHM + 2*theta_0)) &&
				((detector_Angular_Width_FWHM == beam_Angular_Divergence_FWHM) || (beam_Angular_Divergence_FWHM < detector_Angular_Width_FWHM))) || ((beam_Angular_Divergence_FWHM > detector_Angular_Width_FWHM) &&
				((detector_Angular_Width_FWHM + 2*theta_0) >= (beam_Angular_Divergence_FWHM + 2*theta)))))
			{
				integral = 0.5*(detector_Angular_Width_FWHM + beam_Angular_Divergence_FWHM + 2*theta - 2*theta_0);
			} else
			if( ((detector_Angular_Width_FWHM + beam_Angular_Divergence_FWHM + 2*theta_0) > 2*theta) &&
				(((detector_Angular_Width_FWHM == beam_Angular_Divergence_FWHM) &&
				((detector_Angular_Width_FWHM + 2*theta) > (beam_Angular_Divergence_FWHM + 2*theta_0))) || ((beam_Angular_Divergence_FWHM < detector_Angular_Width_FWHM) &&
				((detector_Angular_Width_FWHM + 2*theta_0) <= (beam_Angular_Divergence_FWHM + 2*theta))) || ((beam_Angular_Divergence_FWHM > detector_Angular_Width_FWHM) &&
				((detector_Angular_Width_FWHM + 2*theta) >= (beam_Angular_Divergence_FWHM + 2*theta_0)))))
			{
				integral = 0.5*(detector_Angular_Width_FWHM + beam_Angular_Divergence_FWHM - 2*theta + 2*theta_0);
			} else
		...
		} else
		{
			...
			{
				return detector_Distribution    ( detector_Angular_Width_FWHM, theta_a-theta  ) *
					   beam_Angular_Distribution(beam_Angular_Divergence_FWHM, theta_a-theta_0);
			};
	...
	auto f_S = [&](double point_Index)
	{
		...
		} else
		{
			...
			{
				return detector_Distribution(detector_Angular_Width_FWHM, theta_i-theta) * Global_Variables::beam_Profile(theta_i-theta_0, beam_Spot_FWHM_Angular, beam_Spot_Smoothing)/(beam_Spot_FWHM_Angular*spot_Norm_Factor);
			};
			integral = gauss_kronrod<double,31>::integrate(f_Spot_and_Detector, left_Bound_i, right_Bound_i, 3, 1e-7);
		}
		calculated_Values.S_Instrumental[point_Index] += integral*calculated_Values.R.front();
	};
	...
	auto f_SD = [&](double point_Index)
	{
		...
		auto f_Divergence_and_Spot_and_Detector = [&](double theta_i)
		{
			...
			{
				return Global_Variables::beam_Profile(theta_a-theta_i, beam_Spot_FWHM_Angular, beam_Spot_Smoothing)/beam_Spot_FWHM_Angular * beam_Angular_Distribution(beam_Angular_Divergence_FWHM, theta_a-theta_0)*beam_Angular_Divergence_Norm_Factor;
			};
			double left_Bound_a  = max(-spot_Range_Main_Limit*beam_Spot_FWHM_Angular + theta_i, -beam_Angular_Divergence_Range_Limit*beam_Angular_Divergence_FWHM + theta_0);
			double right_Bound_a = min( spot_Range_Main_Limit*beam_Spot_FWHM_Angular + theta_i,  beam_Angular_Divergence_Range_Limit*beam_Angular_Divergence_FWHM + theta_0);

			if(left_Bound_a>=right_Bound_a) return 0.;

			return detector_Distribution(detector_Angular_Width_FWHM, theta_i-theta) * gauss_kronrod<double,31>::integrate(f_Divergence_and_Spot, left_Bound_a, right_Bound_a, 3, 1e-7);
		};

		double left_Bound_i  = theta - detector_Range_Limit*detector_Angular_Width_FWHM;
		double right_Bound_i = theta + detector_Range_Limit*detector_Angular_Width_FWHM;
	...
	{
		Q_UNUSED(thread_Index)
		if(beam_Angular_Divergence_FWHM>DBL_EPSILON && beam_Spot_FWHM_Angular>DBL_EPSILON && use_beam_spot_in_specular_peak)
		{
			for(int point_Index=n_Min; point_Index<n_Max; ++point_Index)
			{
				f_SD(point_Index);
...
void Main_Calculation_Module::wrap_With_Specular_2D(Calculated_Values& calculated_Values, const Data& measurement)
{
	...
	if(measurement.beam_Theta_0_Distribution.distribution_Function == distributions[Gaussian])	{beam_Distribution_Theta_0 = Global_Variables::distribution_Gaussian;	beam_Norm_Factor_Theta_0 = sqrt(log(16.)/M_PI)/beam_FWHM_Theta_0;	beam_Range_Limit_Theta_0 = 3;    } else return;
	...
	if(measurement.beam_Phi_0_Distribution.distribution_Function == distributions[Gaussian]){beam_Distribution_Phi_0 = Global_Variables::distribution_Gaussian; beam_Norm_Factor_Phi_0 = sqrt(log(16.)/M_PI)/beam_FWHM_Phi_0;	beam_Range_Limit_Phi_0 = 3;    } else return;
	...
	if(measurement.theta_Distribution == distributions[Gaussian])	{detector_Distribution_Theta = Global_Variables::distribution_Gaussian;	detector_Range_Limit_Theta = 3;    } else return;
	...
	if(measurement.phi_Distribution == distributions[Gaussian])	{detector_Distribution_Phi = Global_Variables::distribution_Gaussian;	detector_Range_Limit_Phi = 3;    } else return;
	...
	auto f_Theta = [&](double theta_Index)
	{
		...
		double integral_Theta = detector_Distribution_Theta(detector_FWHM_Theta, theta_0-theta);
	...
	auto f_Theta_D = [&](double theta_Index)
	{
		...
		auto f_Divergence_and_Detector_Theta = [&](double theta_a)
		{
			return detector_Distribution_Theta(detector_FWHM_Theta, theta_a-theta) * beam_Distribution_Theta_0(beam_FWHM_Theta_0, theta_a-theta_0) * beam_Norm_Factor_Theta_0;
		};

		double distance_t_t0 = theta - theta_0;
		double left_Bound_a  = max(theta - detector_Range_Limit_Theta*detector_FWHM_Theta, theta_0 - beam_Range_Limit_Theta_0*beam_FWHM_Theta_0);
		double right_Bound_a = min(theta + detector_Range_Limit_Theta*detector_FWHM_Theta, theta_0 + beam_Range_Limit_Theta_0*beam_FWHM_Theta_0);
		...
		{
			integral_Theta = beam_Norm_Factor_Theta_0 * detector_FWHM_Theta * beam_FWHM_Theta_0 * sqrt_pi * pow(2.,-1-4*distance_t_t0*distance_t_t0/(detector_FWHM_Theta*detector_FWHM_Theta + beam_FWHM_Theta_0*beam_FWHM_Theta_0)) / sqrt((detector_FWHM_Theta*detector_FWHM_Theta + beam_FWHM_Theta_0*beam_FWHM_Theta_0));
		} else
		...
		{
			integral_Theta = beam_Norm_Factor_Theta_0 * 0.25 * beam_FWHM_Theta_0 * sqrt_pi * (erf(log_2*(detector_FWHM_Theta-2*distance_t_t0)/beam_FWHM_Theta_0)+erf(log_2*(detector_FWHM_Theta+2*distance_t_t0)/beam_FWHM_Theta_0));
		} else
		...
		{
			integral_Theta = beam_Norm_Factor_Theta_0 * 0.25 * detector_FWHM_Theta * sqrt_pi * (erf(log_2*(beam_FWHM_Theta_0-2*distance_t_t0)/detector_FWHM_Theta)+erf(log_2*(beam_FWHM_Theta_0+2*distance_t_t0)/detector_FWHM_Theta));
		} else
		{
			integral_Theta = gauss_kronrod<double,15>::integrate(f_Divergence_and_Detector_Theta, left_Bound_a, right_Bound_a, 3, 1e-7);
		}
		return integral_Theta;
	};
	/// detector and spot (no divergence)
	auto f_Theta_S = [&](double theta_Index)
	{
		double theta_0 = measurement.beam_Theta_0_Angle_Value;
		double theta = measurement.detector_Theta_Angle_Vec[theta_Index];
		auto f_Spot_and_Detector_Theta = [&](double theta_i)
		{
			return detector_Distribution_Theta(detector_FWHM_Theta, theta_i-theta) * Global_Variables::beam_Profile(theta_i-theta_0, beam_Spot_FWHM_Theta_0, beam_Spot_Smoothing)/(beam_Spot_FWHM_Theta_0*spot_Norm_Factor);
		};

		double left_Bound_i  = max(theta - detector_Range_Limit_Theta*detector_FWHM_Theta, theta_0 - spot_Range_Limit*beam_Spot_FWHM_Theta_0);
		double right_Bound_i = min(theta + detector_Range_Limit_Theta*detector_FWHM_Theta, theta_0 + spot_Range_Limit*beam_Spot_FWHM_Theta_0);

		if(left_Bound_i>=right_Bound_i) return 0.;

		double integral_Theta = gauss_kronrod<double,15>::integrate(f_Spot_and_Detector_Theta, left_Bound_i, right_Bound_i, 3, 1e-7);
		return integral_Theta;
	};
	/// detector and spot and divergence
	auto f_Theta_SD = [&](double theta_Index)
	{
		double theta_0 = measurement.beam_Theta_0_Angle_Value;
		double theta = measurement.detector_Theta_Angle_Vec[theta_Index];
		auto f_Divergence_and_Spot_and_Detector_Theta = [&](double theta_i)
		{
			auto f_Divergence_and_Spot_Theta = [&](double theta_a)
			{
				return Global_Variables::beam_Profile(theta_a-theta_i, beam_Spot_FWHM_Theta_0, beam_Spot_Smoothing)/beam_Spot_FWHM_Theta_0 * beam_Distribution_Theta_0(beam_FWHM_Theta_0, theta_a-theta_0)*beam_Norm_Factor_Theta_0;
			};
			double left_Bound_a  = max(-spot_Range_Limit*beam_Spot_FWHM_Theta_0 + theta_i, -beam_Range_Limit_Theta_0*beam_FWHM_Theta_0 + theta_0);
			double right_Bound_a = min( spot_Range_Limit*beam_Spot_FWHM_Theta_0 + theta_i,  beam_Range_Limit_Theta_0*beam_FWHM_Theta_0 + theta_0);

			if(left_Bound_a>=right_Bound_a) return 0.;

			return detector_Distribution_Theta(detector_FWHM_Theta, theta_i-theta) * gauss_kronrod<double,15>::integrate(f_Divergence_and_Spot_Theta, left_Bound_a, right_Bound_a, 3, 1e-7);
		};

		double left_Bound_i  = theta - detector_Range_Limit_Theta*detector_FWHM_Theta;
		double right_Bound_i = theta + detector_Range_Limit_Theta*detector_FWHM_Theta;

		if(left_Bound_i>=right_Bound_i) return 0.;

		double integral_Theta = gauss_kronrod<double,15>::integrate(f_Divergence_and_Spot_and_Detector_Theta, left_Bound_i, right_Bound_i, 3, 1e-7);
		return integral_Theta;
	};
	...
	auto f_Phi = [&](double phi_Index)
	{
		...
		double integral_Phi = detector_Distribution_Phi(detector_FWHM_Phi, phi);
		return integral_Phi;
	};
	/// detector and angular divergence (no spot)
	...
	{
		double phi = measurement.detector_Phi_Angle_Vec[phi_Index];
		auto f_Divergence_and_Detector_Phi = [&](double phi_a)
		{
			return detector_Distribution_Phi(detector_FWHM_Phi, phi_a-phi) * beam_Distribution_Phi_0(beam_FWHM_Phi_0, phi_a) * beam_Norm_Factor_Phi_0;
		};

		double distance_p_p0 = phi;
		double left_Bound_a  = max(phi - detector_Range_Limit_Phi*detector_FWHM_Phi, - beam_Range_Limit_Phi_0*beam_FWHM_Phi_0);
		double right_Bound_a = min(phi + detector_Range_Limit_Phi*detector_FWHM_Phi, + beam_Range_Limit_Phi_0*beam_FWHM_Phi_0);
		...
		{
			integral_Phi = beam_Norm_Factor_Phi_0 * detector_FWHM_Phi * beam_FWHM_Phi_0 * sqrt_pi * pow(2.,-1-4*distance_p_p0*distance_p_p0/(detector_FWHM_Phi*detector_FWHM_Phi + beam_FWHM_Phi_0*beam_FWHM_Phi_0)) / sqrt((detector_FWHM_Phi*detector_FWHM_Phi + beam_FWHM_Phi_0*beam_FWHM_Phi_0));
		} else
		...
		{
			integral_Phi = beam_Norm_Factor_Phi_0 * 0.25 * beam_FWHM_Phi_0 * sqrt_pi * (erf(log_2*(detector_FWHM_Phi-2*distance_p_p0)/beam_FWHM_Phi_0)+erf(log_2*(detector_FWHM_Phi+2*distance_p_p0)/beam_FWHM_Phi_0));
		} else
		...
		{
			integral_Phi = beam_Norm_Factor_Phi_0 * 0.25 * detector_FWHM_Phi * sqrt_pi * (erf(log_2*(beam_FWHM_Phi_0-2*distance_p_p0)/detector_FWHM_Phi)+erf(log_2*(beam_FWHM_Phi_0+2*distance_p_p0)/detector_FWHM_Phi));
		} else
		{
			integral_Phi = gauss_kronrod<double,15>::integrate(f_Divergence_and_Detector_Phi, left_Bound_a, right_Bound_a, 3, 1e-7);
		}
		return integral_Phi;
	};
	/// detector and spot (no divergence)
	auto f_Phi_S = [&](double phi_Index)
	{
		double phi = measurement.detector_Phi_Angle_Vec[phi_Index];
		auto f_Spot_and_Detector_Phi = [&](double phi_i)
		{
			return detector_Distribution_Phi(detector_FWHM_Phi, phi_i-phi) * Global_Variables::beam_Profile(phi_i, beam_Spot_FWHM_Phi_0, beam_Spot_Smoothing)/(beam_Spot_FWHM_Phi_0*spot_Norm_Factor);
		};

		double left_Bound_i  = max(phi - detector_Range_Limit_Phi*detector_FWHM_Phi, - spot_Range_Limit*beam_Spot_FWHM_Phi_0);
		double right_Bound_i = min(phi + detector_Range_Limit_Phi*detector_FWHM_Phi, + spot_Range_Limit*beam_Spot_FWHM_Phi_0);

		if(left_Bound_i>=right_Bound_i) return 0.;

		double integral_Phi = gauss_kronrod<double,15>::integrate(f_Spot_and_Detector_Phi, left_Bound_i, right_Bound_i, 3, 1e-7);
		return integral_Phi;
	};
	/// detector and spot and divergence
	...
	{
		double phi = measurement.detector_Phi_Angle_Vec[phi_Index];
		auto f_Divergence_and_Spot_and_Detector_Phi = [&](double phi_i)
		{
			auto f_Divergence_and_Spot_Phi = [&](double phi_a)
			{
				return Global_Variables::beam_Profile(phi_a-phi_i, beam_Spot_FWHM_Phi_0, beam_Spot_Smoothing)/beam_Spot_FWHM_Phi_0 * beam_Distribution_Phi_0(beam_FWHM_Phi_0, phi_a)*beam_Norm_Factor_Phi_0;
			};
			double left_Bound_a  = max(-spot_Range_Limit*beam_Spot_FWHM_Phi_0 + phi_i, -beam_Range_Limit_Phi_0*beam_FWHM_Phi_0);
			double right_Bound_a = min( spot_Range_Limit*beam_Spot_FWHM_Phi_0 + phi_i,  beam_Range_Limit_Phi_0*beam_FWHM_Phi_0);

			if(left_Bound_a>=right_Bound_a) return 0.;

			return detector_Distribution_Phi(detector_FWHM_Phi, phi_i-phi) * gauss_kronrod<double,15>::integrate(f_Divergence_and_Spot_Phi, left_Bound_a, right_Bound_a, 3, 1e-7);
		};

		double left_Bound_i  = phi - detector_Range_Limit_Phi*detector_FWHM_Phi;
		double right_Bound_i = phi + detector_Range_Limit_Phi*detector_FWHM_Phi;

		if(left_Bound_i>=right_Bound_i) return 0.;

		double integral_Phi = gauss_kronrod<double,15>::integrate(f_Divergence_and_Spot_and_Detector_Phi, left_Bound_i, right_Bound_i, 3, 1e-7);
		return integral_Phi;
	};
	...
	Global_Variables::parallel_For(int(f_Phi_Vec.size()), reflectivity_calc_threads, [&](int n_Min, int n_Max, int thread_Index)
	{
		if(use_Beam_Spot)
		{
			if(beam_FWHM_Phi_0>DBL_EPSILON && beam_Spot_FWHM_Phi_0>DBL_EPSILON)	 {
				for(int phi_Index=n_Min; phi_Index<n_Max; ++phi_Index)
				{f_Phi_Vec[phi_Index] = f_Phi_SD(phi_Index);}
			}
			if(beam_FWHM_Phi_0<=DBL_EPSILON && beam_Spot_FWHM_Phi_0>DBL_EPSILON) {
				for(int phi_Index=n_Min; phi_Index<n_Max; ++phi_Index)
				{f_Phi_Vec[phi_Index] = f_Phi_S(phi_Index);}
			}
		}
		if(beam_FWHM_Phi_0>DBL_EPSILON && (!use_Beam_Spot || beam_Spot_FWHM_Phi_0<=DBL_EPSILON))	{
			for(int phi_Index=n_Min; phi_Index<n_Max; ++phi_Index)
	...
	Global_Variables::parallel_For(int(measurement.detector_Theta_Angle_Vec.size()), reflectivity_calc_threads, [&](int n_Min, int n_Max, int thread_Index)
	{
		...
		for(int theta_Index=n_Min; theta_Index<n_Max; ++theta_Index)
		{
			...
			if(use_Beam_Spot)
			{
				if(beam_FWHM_Theta_0>DBL_EPSILON && beam_Spot_FWHM_Theta_0>DBL_EPSILON)	  {
					f_Theta_Val = f_Theta_SD(theta_Index);
				}
				if(beam_FWHM_Theta_0<=DBL_EPSILON && beam_Spot_FWHM_Theta_0>DBL_EPSILON)  {
					f_Theta_Val = f_Theta_S(theta_Index);
				}
			}
			if(beam_FWHM_Theta_0>DBL_EPSILON && (!use_Beam_Spot || beam_Spot_FWHM_Theta_0<=DBL_EPSILON))  {
