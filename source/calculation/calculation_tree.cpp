		...
		if(child_Data.item_Type == item_Type_Regular_Aperiodic)
		{
			for(int period_Index=0; period_Index<child_Data.num_Repetition.value(); ++period_Index)
			{
				for(int cell_Index=0; cell_Index<child_Data.regular_Components.size(); ++cell_Index)
				{
					...
					media_Data_Map_Vector[media_Index].aperiodic_Avatar_Id = child_Data.regular_Components[cell_Index].top_Id;
					++media_Index;
				}
			}
		}
...
void Calculation_Tree::calculate_1_Kind_Preliminary(Data_Element<Type>& data_Element)
{
	...
	if(data_Element.curve_Class == TARGET)
	{
		...
		if( target_Curve->curve.value_Type == value_Types[Reflectance]   )	{ data_Element.calc_Functions.check_Reflectance = true;		target_Curve->calc_Functions.check_Reflectance = true;	}
		if( target_Curve->curve.value_Type == value_Types[Transmittance] )	{ data_Element.calc_Functions.check_Transmittance = true;	target_Curve->calc_Functions.check_Transmittance = true;}
		if( target_Curve->curve.value_Type == value_Types[Scattering]   )	{ data_Element.calc_Functions.check_Scattering = true;		target_Curve->calc_Functions.check_Scattering = true;	}
		if( target_Curve->curve.value_Type == value_Types[GISAS]   )		{ data_Element.calc_Functions.check_GISAS = true;			target_Curve->calc_Functions.check_GISAS = true;		}
	}
	...
	for(size_t node_Index = 0; node_Index<short_Flat_Calc_Tree.size(); node_Index++)
	{
		if( mode == SCATTERED_MODE )
		{
			if(multilayer->imperfections_Model.use_Roughness)
			{
				...
				{
					short_Flat_Calc_Tree[node_Index]->create_Spline_PSD_Peak			(multilayer->imperfections_Model, measurement);
						...
						multilayer->imperfections_Model.vertical_Correlation == zero_Correlation)
					{
						...
						if(short_Flat_Calc_Tree[node_Index]->spline_PSD_Combined_1D_Condition	(multilayer->imperfections_Model))
						{
							if(!measurement.detector_1D.finite_Slit) {
								short_Flat_Calc_Tree[node_Index]->create_Spline_PSD_Fractal_Gauss_2D(multilayer->imperfections_Model);
							}
					...
					if(multilayer->imperfections_Model.vertical_Correlation == partial_Correlation)
					{
						...
						short_Flat_Calc_Tree[node_Index]->create_Spline_PSD_Measured		    (multilayer->imperfections_Model);
						short_Flat_Calc_Tree[node_Index]->create_Spline_PSD_Linear_Growth_2D	(multilayer->imperfections_Model, measurement, media_Data_Map_Vector, 0);
						short_Flat_Calc_Tree[node_Index]->create_Spline_PSD_Linear_Growth_1D	(multilayer->imperfections_Model, measurement);
					}
...
//					short_Flat_Calc_Tree[node_Index]->create_Spline_PSD_Fractal_Gauss_1D(multilayer->imperfections_Model, measurement);
		...
		// here we calculate total sigma for specular direction in SPECULAR_MODE
		{
			if(multilayer->imperfections_Model.use_Roughness)
			{
					...
					measurement.measurement_Type == measurement_Types[GISAS_Map])
				{
					...
					short_Flat_Calc_Tree[node_Index]->calc_Debye_Waller_Total_Sigma(multilayer->imperfections_Model);
				}
			}
		}
	}
	for(size_t node_Index = 0; node_Index<short_Flat_Calc_Tree.size(); node_Index++)
	{
		if( measurement.measurement_Type == measurement_Types[Specular_Scan])
		{
			if(multilayer->imperfections_Model.use_Roughness)
			{
				...
				if((multilayer->imperfections_Model.vertical_Correlation == partial_Correlation &&
				   (multilayer->imperfections_Model.inheritance_Model == linear_Growth_Alpha_Inheritance_Model ||
					...
					measurement.detector_1D.finite_Slit)
				{
					short_Flat_Calc_Tree[node_Index]->create_Spline_PSD_Fractal_Gauss_2D    (multilayer->imperfections_Model);
					short_Flat_Calc_Tree[node_Index]->create_Spline_PSD_Fractal_Gauss_1D    (multilayer->imperfections_Model, measurement);
				}
			...
			short_Flat_Calc_Tree[node_Index]->calc_Debye_Waller_Sigma(multilayer->imperfections_Model, measurement);
		}
	}
}
			...
			if(multilayer->imperfections_Model.use_Roughness)
			{
					...
					measurement.measurement_Type == measurement_Types[Offset_Scan])
				{
					...
					if(multilayer->imperfections_Model.vertical_Correlation == partial_Correlation)
					{
						...
						short_Flat_Calc_Tree[node_Index]->clear_Spline_PSD_Linear_Growth_2D(multilayer->imperfections_Model);
						short_Flat_Calc_Tree[node_Index]->clear_Spline_PSD_Linear_Growth_1D(multilayer->imperfections_Model);
					}
				}
				if(measurement.measurement_Type == measurement_Types[GISAS_Map])
				{
