...
void Node::calculate_Intermediate_Points(const Data& measurement, Node* above_Node, bool depth_Grading, bool sigma_Grading, bool enable_Discretization)
{
	// PARAMETER
	{
			...
			struct_Data.item_Type == item_Type_Substrate )
		{
			...
			{
				// scattered points
				num_Points = measurement.detector_Theta_Cos2_Vec.size()+1;  // scattered + 1 specular point
				cos2 = measurement.detector_Theta_Cos2_Vec; // last point we add

				// last point is specular
				cos2.push_back(measurement.beam_Theta_0_Cos2_Value);

				// common
				k.resize(num_Points);		// num scattered points(many) >= num specular points(==1)
				for(int point_Index = 0; point_Index<num_Points; ++point_Index)
				{
					k[point_Index] = measurement.k_Value;
				}

				epsilon.resize(num_Points);  // num scattered points(many) >= num specular points(==1)
				vector<double> spectral_Points (1, measurement.wavelength.value);
				fill_Epsilon_For_Angular_Measurements(spectral_Points);
			}
