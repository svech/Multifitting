	...
	if(active_Whats_This_List[1] == whats_This_Angle)
	{
		...
		if(whats_This_List[0] == whats_This_Ambient)
		{
			...
			// compile from elements
			{
				...
				int status = optical_Constants->make_Epsilon_From_Factors(ambient.composition, ambient.absolute_Density.value, spectral_Points, temp_Epsilon, warning_Text);
				if(status!=0)
				{
					return status;
				}
...
//					corresponding_Flat_Node.plot_Points[i].epsilon = plot_Points[i].epsilon;
				...
				/// no interferentional matrix for ambient
...
//				corresponding_Flat_Node.plot_Points[i].weak_Factor = plot_Points[i].weak_Factor;
				...
				if(measur->polarization.value > -1)
				{
					...
					plot_Points[i].interference_Matrix_s(1,1) =      plot_Points[i].exponenta                                / fresnel_Square_s;
				}
...
//					corresponding_Flat_Node.plot_Points[i].epsilon = plot_Points[i].epsilon;
				...
				/// exponent
				/// no really existing exponent for substrate, just "effective" value
