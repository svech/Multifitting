...
public:
	...
	void reload();
	int interpolation_Epsilon    (QVector<Point>& input_Values,                      QVector<double>& spectral_Points, QVector<complex<double>>& output_Values, QString& error_Text, QString material);
	int make_Epsilon_From_Factors(QList<Stoichiometry>& composition, double density, QVector<double>& spectral_Points, QVector<complex<double>>& epsilon, QString& error_Text);
private:
	void read_All();
